...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	% Plot bars
	handles.bars = bar(barvalues, width,'edgecolor',1-axcol, 'linewidth', 2);
	hold on
	...
	for i = 1:numbars
		...
		handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i), 'linestyle', 'none', 'linewidth', 2,'color',1-axcol);
		ymax = max([ymax; barvalues(:,i)+errors(:,i)]);
        
    end
    ymin=min(barvalues(:)-errors(:));
	...
	ylim([min(ymin*1.1,0) max(0,ymax*1.1)]);
