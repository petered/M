...
function hLink=linkmaxes(h,type)
% Improvement on the built in linkaxes function - it scales to the maximum
    ...
    for i=1:length(type)
        if ismember(type(i),'xyzc')
            hLink=linkmprop(h,[type(i),'lim'],true);
        else
...
function hLink=linkmprop(h,prop,fast)
    if nargin<3, fast=false; end

    arr=get(h,prop);
    if numel(h)>1, 
        if fast && length(arr)>1000
            arr=cell2mat(arr(ceil(rand(1,1000)*length(arr)))); 
        else
            arr=cell2mat(arr); 
        end
    end
