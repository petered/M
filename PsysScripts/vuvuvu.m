...
function output_signal=vuvuvu(y,Fe,t)

% fprintf('--- Vuvuzela Cancelation Program ---\n\n');

% t can be either a time-period (eg [1 2]) or a signal.  If it only has 2
% elements, it'll assumed to be a time period.  If it has more, then a
% signal.
...
if numel(t)==2
    ...
    Snoise = spectrogram(x+i*eps,window,window_length-overlap,NFFT,Fe); %put a short imaginary part to obtain two-sided spectrogram

    absS_vuvuzela=abs(Snoise).^2;
